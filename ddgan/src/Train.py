...
class GAN:
    ...
    def __init__(self):
        ...
        self.generator_opt = tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0, beta_2=0.9)
        self.discriminator_opt = tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0, beta_2=0.9)
    ...
    def setup(self, kwargs) -> None:
        ...
        Args:
            kwargs (dict): key-value pairs of some stuff
        """
        self.kwargs = kwargs
    ...
    def make_generator(self):  # nsteps):
        ...
        self.generator.add(tf.keras.layers.BatchNormalization())
        self.generator.add(tf.keras.layers.Dense((5*self.nsteps), activation='tanh'))                # 25
        return None
    ...
    def make_discriminator(self):  # nsteps):
        ...
        Discriminator
        """
        self.discriminator.add(tf.keras.layers.Dense(5*self.nsteps, input_shape=(5*self.nsteps,)))
    ...
    def train_step(self, noise, real):
        ...
        for i in range(self.n_critic):
            ...
            new_d_loss = self.update_discriminator_loss(d_loss, batch_size, fake, real)
            c_grad = t.gradient(new_d_loss, self.discriminator.trainable_variables)
            self.discriminator_opt.apply_gradients(zip(c_grad, self.discriminator.trainable_variables))

        # train generator
        with tf.GradientTape() as gen_tape:
            fake_images = self.generator(noise, training=True)
            d_fake = self.discriminator(fake_images, training=True)     # training=False?
    ...
    def train(self, batch_size, batches, training_data, input_to_GAN, epochs):
        ...
        for epoch in range(epochs):
            ...
            xx1 = real_data.reshape(batches, batch_size, self.ndims*self.nsteps)
            ...
            losses[epoch,:] = [epoch+1, self.g_loss.result().numpy(), self.d_loss.result().numpy(), self.w_loss.result().numpy()]
