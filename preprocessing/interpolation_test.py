...
value_grid = u2r.simple_interpolate_from_mesh_to_grid(value_mesh,x_all,x_ndgln,ddx,block_x_start,nx,ny,nz,nEl,nloc,nNodes,nscalar,ndim,nTime) 
...
filename = 'LSBU_' + str(i) + '_structured.vtu'
ug=vtktools.vtu(filename)
ug.AddScalarField('Claire', np.squeeze(value_grid))
print('np.squeeze(value_grid).shape', np.squeeze(value_grid).shape)
ug.Write()
