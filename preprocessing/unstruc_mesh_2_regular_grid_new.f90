...
!          call simple_interpolate_from_mesh_to_grid_one(value_grid, value_mesh, x_all,x_ndgln,ddx,block_x_start, &
      ...
      subroutine simple_interpolate_from_mesh_to_grid_old(vg,vm,x,xgn,ddx,start,nx,ny,nz,nEl,nloc,nNodes, nscalar,ndim,ntime) 
! *******************************************************************************************************
...
! *******************************************************************************************************
      implicit none
      integer, intent(in) :: nx,ny,nz,nNodes,nEl,nloc,nscalar,ndim,ntime
...
! elewic_keep,l1234_keep are the same as  elewic,l1234 but with stored values of these.
      ...
      call interpolate_from_mesh_to_grid(value_grid, elewic,l1234,value_mesh, &
                                         igot_ele_store,elewic_keep,l1234_keep, &
                                         x_all,x_ndgln,ddx,block_x_start,nx,ny,nz, &
...
! 
! in python:
! value_remesh = u2r.interpolate_from_grid_to_mesh(value_grid, block_x_start, ddx, x_all, nscalar,nx,ny,nz,nNodes,ndim,nTime)
...
! local variables...
      ...
      integer node, i,j,k, ii,jj,kk, nnx(ndim)
